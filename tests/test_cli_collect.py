...
def jsonl_file(tmp_path: Path) -> Path:
    ...
    records = [
        ...
        _make_episode(goal_achieved=False, max_prediction_error=0.25,
                      internal_flag_raised=True, correction_source="self",
                      error_before=0.30, error_after=0.08),
        _make_episode(goal_achieved=True,  max_prediction_error=0.10),
        _make_episode(goal_achieved=False, max_prediction_error=0.20,
                      internal_flag_raised=True, correction_source="harness"),
...
class TestCollectIngest:
    ...
    def test_loads_jsonl_and_shows_stats(self, jsonl_file: Path):
        result = runner.invoke(app, [
            "collect", "ingest", str(jsonl_file), "--dry-run",
        ])
        assert result.exit_code == 0
    ...
    def test_loads_json_array(self, json_array_file: Path):
        result = runner.invoke(app, [
            "collect", "ingest", str(json_array_file), "--dry-run",
        ])
        assert result.exit_code == 0
    ...
    def test_agent_id_override(self, jsonl_file: Path):
        result = runner.invoke(app, [
            "collect", "ingest", str(jsonl_file),
            "--agent-id", "overridden-agent",
            "--dry-run",
        ])
        assert result.exit_code == 0
    ...
    def test_multiple_files(self, tmp_path: Path, jsonl_file: Path):
        ...
        result = runner.invoke(app, [
            "collect", "ingest", str(jsonl_file), str(second), "--dry-run",
        ])
        assert result.exit_code == 0

    def test_missing_file_exits_1(self, tmp_path: Path):
        result = runner.invoke(app, [
            "collect", "ingest", str(tmp_path / "nonexistent.jsonl"),
        ])
...
class TestCollectReplay:
    ...
    def test_replay_missing_file_exits_1(self, tmp_path: Path):
        result = runner.invoke(app, [
            "collect", "replay", str(tmp_path / "none.jsonl"),
        ])
        assert result.exit_code == 1
...
class TestCollectStats:
    ...
    def test_scr_calculation(self, tmp_path: Path):
        ...
        records = [
            # 분모 포함, 자기수정 성공
            _make_episode(max_prediction_error=0.25, internal_flag_raised=True,
                          correction_source="self", error_before=0.30, error_after=0.08),
            # 분모 포함, 하네스 수정 (SCR 제외)
            _make_episode(goal_achieved=False, internal_flag_raised=True,
    ...
    def test_missing_file_exits_1(self, tmp_path: Path):
        result = runner.invoke(app, [
            "collect", "stats", str(tmp_path / "nope.jsonl"),
        ])
        assert result.exit_code == 1

    def test_show_episodes_flag(self, jsonl_file: Path):
        result = runner.invoke(app, [
            "collect", "stats", str(jsonl_file), "--episodes",
        ])
        assert result.exit_code == 0
    ...
    def test_multiple_files_merged(self, tmp_path: Path, jsonl_file: Path):
        ...
        result = runner.invoke(app, [
            "collect", "stats", str(jsonl_file), str(second),
        ])
        assert result.exit_code == 0
        # 5 + 1 = 6건
