...
# 조항별 지표가 하나도 측정되지 않았을 때 적용하는 보수적 기본 점수
# (실제 준수 증거 없음 → partial 경계 이하로 설정)
_UNMEASURED_ARTICLE_SCORE: float = 40.0
...
class EUAIActMapper:
    ...
    def _map_art13(self, report: DiagnosticReport) -> ArticleMapping:
        ...
        if ece:
            ...
            if ece.status == "critical":
                ...
                recommendations.append(
                    "보정(calibration) 재훈련 또는 Temperature Scaling 적용 권장"
                )
        ...
        compliance_score = (sum(scores) / len(scores)) if scores else _UNMEASURED_ARTICLE_SCORE
    ...
    def generate_monitoring_report(
        self, report: DiagnosticReport, output_format: str = "html"
    ) -> str:
...
"""
        ...
        for art in eu_report.articles:
            ...
            findings_html = "".join(
                f'<div class="finding">⚠ {html.escape(f)}</div>' for f in art.findings
            )
            recs_html = "".join(
                f'<div class="rec">→ {html.escape(r)}</div>' for r in art.recommendations
            )
            parts.append(
                f"""<div class="article">
<h2>{html.escape(art.article)} — {html.escape(art.title)}</h2>
<p>{html.escape(art.description)}</p>
<p><strong>모니터링 점수:</strong> {art.compliance_score:.1f}/100
&nbsp;{self._status_badge(art.status)}</p>
...
"""
        ...
        parts.append(
            """<footer>
본 보고서는 HAchillesWorld SDK가 자동 생성한 <strong>모니터링 참고 자료</strong>입니다.<br>
EU AI Act 법적 준수 여부는 공인된 적합성 평가 기관 및 법률 전문가 검토가 필요합니다.<br>
본 보고서를 규제 기관 제출용 법적 증거 자료로 단독 사용하지 마십시오.
    ...
    def _render_text(self, eu_report: EUAIActReport) -> str:
        lines = [
            ...
            (
                f"종합 모니터링 점수: "
                f"{eu_report.overall_compliance_score:.1f}/100 [{eu_report.overall_status}]"
            ),
            "=" * 60,
        ]
        for art in eu_report.articles:
