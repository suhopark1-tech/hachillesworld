...
Art.13 투명성, Art.14 인간 감독, Art.15 정확성·견고성을 HAchillesWorld 진단
지표에 자동 매핑하고 HTML 보고서를 생성한다.
"""
...
class EUAIActMapper:
    """DiagnosticReport → EU AI Act Art.13~15 자동 매핑."""

    def map_to_articles(self, report: DiagnosticReport) -> EUAIActReport:
        """HAchillesWorld 진단 리포트를 EU AI Act 조항에 매핑한다."""
...
<head>
<meta charset="UTF-8">
<title>EU AI Act 컴플라이언스 보고서 — {html.escape(eu_report.agent_name)}</title>
<style>
...
<p><strong>준수 점수:</strong> {art.compliance_score:.1f}/100 &nbsp;{self._status_badge(art.status)}</p>
...
"""
        ...
        parts.append(
            """<footer>
본 보고서는 HAchillesWorld SDK v2.0 EUAIActMapper가 자동 생성했습니다.<br>
EU AI Act 최종 법적 준수 판단은 자격을 갖춘 법률 전문가의 검토가 필요합니다.
</footer>
</body></html>"""
        )
        return "\n".join(parts)
    ...
    def _render_text(self, eu_report: EUAIActReport) -> str:
        lines = [
            "=" * 60,
            "EU AI Act 컴플라이언스 보고서",
            f"에이전트: {eu_report.agent_name}",
            f"생성일시: {eu_report.generated_at}",
            f"종합 준수 점수: {eu_report.overall_compliance_score:.1f}/100 [{eu_report.overall_status}]",
            "=" * 60,
        ]
        for art in eu_report.articles:
            lines += [
